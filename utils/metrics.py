...
def eval_func(distmat, q_pids, g_pids, q_camids, g_camids, q_clothids=None, g_clothids=None, max_rank=50):
...
class R1_mAP_CC_eval():
    def __init__(self, num_query, max_rank=50, feat_norm=True, reranking=False):
        super(R1_mAP_CC_eval, self).__init__()
        self.num_query = num_query
        self.max_rank = max_rank
        self.feat_norm = feat_norm
        self.reranking = reranking
    ...
    def compute(self): 
        ...
        if self.reranking:
            ...
            print('=> Computing DistMat with euclidean_distance')
            distmat = euclidean_distance(qf, gf)
        cmc, mAP = eval_func(distmat, q_pids, g_pids, q_camids, g_camids, q_clothids=q_clothids, g_clothids=g_clothids)
